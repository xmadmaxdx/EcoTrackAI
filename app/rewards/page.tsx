...
export default function RewardsPage() {
        ...
        if (userEmail) {
          ...
          if (fetchedUser) {
            ...
            const earned = fetchedTransactions
              .filter(t => t.type === 'earned_report' || t.type === 'earned_collect')
              .reduce((acc, t) => acc + t.amount, 0);
            const redeemed = fetchedTransactions
              .filter(t => t.type === 'redeemed')
              .reduce((acc, t) => acc + t.amount, 0);
            const calculatedBalance = earned - redeemed;
  ...
  const handleRedeemReward = async (rewardId: number) => {
    ...
    // Log all values to inspect them
    console.log("Redeeming reward:", reward);
    console.log("Current balance:", currentBalance);
    console.log("Reward cost:", rewardCost);
  
    if (reward && currentBalance >= rewardCost && rewardCost > 0) {
      try {
        // Extra check (this line might be redundant)
        ...
        await createTransaction(user.id, 'redeemed', rewardCost, `Redeemed ${reward.name}`);
